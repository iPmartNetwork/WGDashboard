...
update_wgd() {
  new_ver=$(python3 -c "import json; import urllib.request; data = urllib.request.urlopen('https://api.github.com/repos/ipmartnetwork/WGDashboard.git').read(); output = json.loads(data);print(output['tag_name'])")
  printf "%s\n" "$dashes"
