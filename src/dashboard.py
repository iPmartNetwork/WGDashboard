...
# --- User Role Model & Table ---
def create_admin_table():
    with sqlite3.connect(os.path.join(CONFIGURATION_PATH, 'db', 'wgdashboard.db')) as db:
        db.execute('''
...
def get_user_by_username(username):
    with sqlite3.connect(os.path.join(CONFIGURATION_PATH, 'db', 'wgdashboard.db')) as db:
        ...
        cur = db.cursor()
        cur.execute("SELECT * FROM AdminUsers WHERE username = ?", (username,))
        return cur.fetchone()

def add_admin_user(username, password, role='admin'):
    hashed = bcrypt.hashpw(password.encode("utf-8"), bcrypt.gensalt()).decode("utf-8")
    with sqlite3.connect(os.path.join(CONFIGURATION_PATH, 'db', 'wgdashboard.db')) as db:
        db.execute("INSERT INTO AdminUsers (username, password, role) VALUES (?, ?, ?)", (username, hashed, role))

def update_admin_user_role(username, role):
    with sqlite3.connect(os.path.join(CONFIGURATION_PATH, 'db', 'wgdashboard.db')) as db:
        db.execute("UPDATE AdminUsers SET role = ? WHERE username = ?", (role, username))

def delete_admin_user(username):
    with sqlite3.connect(os.path.join(CONFIGURATION_PATH, 'db', 'wgdashboard.db')) as db:
        db.execute("DELETE FROM AdminUsers WHERE username = ?", (username,))
...
def get_all_admin_users():
    with sqlite3.connect(os.path.join(CONFIGURATION_PATH, 'db', 'wgdashboard.db')) as db:
        db.row_factory = sqlite3.Row
        cur = db.cursor()
        cur.execute("SELECT username, role FROM AdminUsers")
...
# --- Admin User APIs ---
@app.post(f'{APP_PREFIX}/api/admin/login')
def API_Admin_Login():
    data = request.get_json()
    ...
    if user and bcrypt.checkpw(password.encode("utf-8"), user["password"].encode("utf-8")):
        ...
        return ResponseObject(True, "Login successful", data={"role": user["role"]})
    return ResponseObject(False, "Invalid credentials", status_code=401)

@app.get(f'{APP_PREFIX}/api/admin/logout')
def API_Admin_Logout():
    session.pop("admin_username", None)
    return ResponseObject(True, "Logged out")

@app.get(f'{APP_PREFIX}/api/admin/users')
@require_role(["superadmin"])
def API_Admin_ListUsers():
    return ResponseObject(True, data=get_all_admin_users())

@app.post(f'{APP_PREFIX}/api/admin/add')
@require_role(["superadmin"])
def API_Admin_AddUser():
    data = request.get_json()
    username = data.get("username")
    password = data.get("password")
    role = data.get("role", "admin")
    if not username or not password:
        return ResponseObject(False, "Username and password required")
    try:
        add_admin_user(username, password, role)
        return ResponseObject(True, "User added")
    except Exception as e:
        return ResponseObject(False, str(e))

@app.post(f'{APP_PREFIX}/api/admin/updateRole')
@require_role(["superadmin"])
def API_Admin_UpdateRole():
    data = request.get_json()
    username = data.get("username")
    role = data.get("role")
    if not username or not role:
        return ResponseObject(False, "Username and role required")
    update_admin_user_role(username, role)
    return ResponseObject(True, "Role updated")

@app.post(f'{APP_PREFIX}/api/admin/delete')
@require_role(["superadmin"])
def API_Admin_DeleteUser():
    data = request.get_json()
    username = data.get("username")
    if not username:
        return ResponseObject(False, "Username required")
    delete_admin_user(username)
    return ResponseObject(True, "User deleted")

# --- Live Bandwidth & Server Status APIs ---
@app.get(f'{APP_PREFIX}/api/monitor/bandwidth')
@require_role(["admin", "superadmin"])
def API_Monitor_Bandwidth():
    # نمونه ساده: جمع کل ارسال و دریافت همه اینترفیس‌ها
    stats = psutil.net_io_counters(pernic=True)
    now = datetime.now().strftime("%H:%M:%S")
    total_sent = sum([iface.bytes_sent for iface in stats.values()])
    total_recv = sum([iface.bytes_recv for iface in stats.values()])
    # می‌توانید داده‌های قبلی را ذخیره و نرخ را حساب کنید
    return ResponseObject(True, data=[{"time": now, "value": round((total_sent + total_recv) / 1024 / 1024, 2)}])

@app.get(f'{APP_PREFIX}/api/monitor/server-status')
@require_role(["admin", "superadmin"])
def API_Monitor_ServerStatus():
    now = datetime.now().strftime("%H:%M:%S")
    cpu = psutil.cpu_percent(interval=0.5)
    ram = psutil.virtual_memory().percent
    return ResponseObject(True, data=[{"time": now, "cpu": cpu, "ram": ram}])
